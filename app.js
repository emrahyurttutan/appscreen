...
function setupSliderResetButtons() {
  document
    .querySelectorAll('.control-row input[type="range"]')
    .forEach((slider) => {
      ...
      btn.type = "button";
      btn.innerHTML =
        '<svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M3 12a9 9 0 1 1 3 6.75"/><polyline points="3 16 3 10 9 10"/></svg>';
...
function setCurrentScreenshotAsDefault() {
  ...
  if (screenshot) {
    state.defaults.background = JSON.parse(
      JSON.stringify({ ...screenshot.background, image: undefined }),
    );
    // Reconstruct background Image from data URL
    if (state.defaults.background.imageSrc) {
      const bgImg = new Image();
      bgImg.onload = () => {
        state.defaults.background.image = bgImg;
      };
...
// Loaded Image objects for project logos (not persisted, rebuilt on load)
const projectLogoImages = {};

function getCurrentProject() {
  return projects.find((p) => p.id === currentProjectId) || projects[0];
}

function getProjectLogoImage(projectId) {
  return projectLogoImages[projectId || currentProjectId] || null;
}

function loadProjectLogoImage(project) {
  if (!project || !project.logo) {
    delete projectLogoImages[project?.id];
    return;
  }
  const img = new Image();
  img.onload = () => {
    projectLogoImages[project.id] = img;
    updateProjectLogoUI();
    if (state.outputDevice === "web-feature") updateCanvas();
  };
  img.src = project.logo;
}

function updateProjectLogoUI() {
  const project = getCurrentProject();
  const logoImg = document.getElementById("project-logo-img");
  const logoPlaceholder = document.getElementById("project-logo-placeholder");
    ...
    try {
      ...
      transaction.oncomplete = () => {
        if (projectsReq.result) {
          ...
          // Ensure new fields exist on legacy projects
          projects.forEach((p) => {
            if (!p.featureTags) p.featureTags = [];
            if (!p.tagline) p.tagline = "";
            if (p.logo) loadProjectLogoImage(p);
          });
        }
...
// Render the feature tags input list in the sidebar
function renderFeatureTagsList() {
  const project = getCurrentProject();
  const list = document.getElementById("feature-tags-list");
  if (!list) return;
  list.innerHTML = "";
  const tags = project && project.featureTags ? project.featureTags : [];
  tags.forEach((tag, i) => {
    const row = document.createElement("div");
    row.className = "feature-tag-row";
    row.innerHTML = `
            <input type="text" class="feature-tag-input" value="${tag.replace(/"/g, "&quot;")}" placeholder="Tag label..." data-index="${i}">
            <button class="feature-tag-remove" data-index="${i}" title="Remove">
                <svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2">
                    <path d="M18 6L6 18M6 6l12 12"/>
                </svg>
            </button>
        `;
    row.querySelector(".feature-tag-input").addEventListener("input", (e) => {
      const p = getCurrentProject();
      if (p) {
        p.featureTags[i] = e.target.value;
        saveProjectsMeta();
        if (state.outputDevice === "web-feature") updateCanvas();
      }
    });
    row.querySelector(".feature-tag-remove").addEventListener("click", () => {
      const p = getCurrentProject();
      if (p) {
        p.featureTags.splice(i, 1);
        saveProjectsMeta();
        renderFeatureTagsList();
        if (state.outputDevice === "web-feature") updateCanvas();
      }
    });
...
// Show/hide feature graphic settings panel and sync values
function updateFeatureGraphicPanel() {
  const panel = document.getElementById("feature-graphic-panel");
  if (!panel) return;
  const isFeatureGraphic = state.outputDevice === "web-feature";
  panel.style.display = isFeatureGraphic ? "block" : "none";
  if (isFeatureGraphic) {
    const project = getCurrentProject();
    const taglineInput = document.getElementById("feature-tagline");
    if (taglineInput && project) taglineInput.value = project.tagline || "";
    renderFeatureTagsList();
  }
}

// Draw feature graphic layout on canvas (1024x500 landscape layout)
function drawFeatureGraphicLayout() {
  const project = getCurrentProject();
  const dims = getCanvasDimensions(); // 1024 x 500
  const W = dims.width;
  const H = dims.height;

  const EDGE = Math.round(W * 0.04); // edge padding ~41px
  const GAP = Math.round(W * 0.035); // zone gap ~36px
  const zoneW = Math.floor((W - 2 * EDGE - 2 * GAP) / 3); // ~287px each zone

  const logoImg = getProjectLogoImage(currentProjectId);
  const projectName = project ? project.name : "App Name";
  const tagline = project ? project.tagline || "" : "";
  const tags = project ? project.featureTags || [] : [];

  // --- Left zone: app icon ---
  const iconSize = Math.min(Math.round(H * 0.58), zoneW - Math.round(H * 0.08));
  const iconX = EDGE + Math.round((zoneW - iconSize) / 2);
  ...
  ctx.shadowBlur = Math.round(iconSize * 0.12);
  ctx.shadowOffsetY = Math.round(iconSize * 0.05);
  ctx.fillStyle = "#ffffff";
  roundRect(ctx, iconX, iconY, iconSize, iconSize, iconRadius);
  ctx.fill();
  ctx.restore();

  // Logo image inside icon area (with padding)
  if (logoImg && logoImg.complete) {
    const logoPad = Math.round(iconSize * 0.1);
    const logoDrawSize = iconSize - logoPad * 2;
    ctx.save();
    ctx.beginPath();
    ctx.roundRect(
      iconX + logoPad,
  ...
  // --- Middle zone: app name + tagline ---
  const midX = EDGE + zoneW + GAP;
  const midMaxW = zoneW;
  const midCenterY = Math.round(H / 2);
  ...
  // App name
  const nameFontSize = Math.round(H * 0.115);
  ctx.font = `700 ${nameFontSize}px -apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif`;
  // Word-wrap project name
  const nameLines = wrapText(ctx, projectName, midMaxW);
  const nameLineHeight = nameFontSize * 1.15;
  const totalNameH = nameLines.length * nameLineHeight;

  const taglineFontSize = Math.round(H * 0.056);
  ctx.font = `400 ${taglineFontSize}px -apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif`;
  const taglineLines = tagline ? wrapText(ctx, tagline, midMaxW) : [];
  const taglineLineHeight = taglineFontSize * 1.3;
  const totalTaglineH = taglineLines.length * taglineLineHeight;

  const gap = tagline ? Math.round(H * 0.04) : 0;
  const totalH = totalNameH + gap + totalTaglineH;
  let startY = midCenterY - totalH / 2 + nameLineHeight / 2;

  // Draw name lines
  ctx.font = `700 ${nameFontSize}px -apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif`;
  ctx.fillStyle = "#ffffff";
  nameLines.forEach((line, li) => {
    ctx.fillText(line, midX, startY + li * nameLineHeight);
  });
  ...
  if (taglineLines.length > 0) {
    ...
    const taglineStartY = startY + totalNameH + gap;
    taglineLines.forEach((line, li) => {
      ctx.fillText(line, midX, taglineStartY + li * taglineLineHeight);
    });
  }
  ctx.restore();

  // --- Right zone: feature tags ---
  if (tags.length > 0) {
    const rightX = EDGE + zoneW + GAP + zoneW + GAP;
    const rightW = zoneW;
    drawFeatureTags(tags, rightX, rightW, H);
  }
}
...
// Helper: draw feature tags grid on the right side
function drawFeatureTags(tags, startX, maxWidth, canvasH) {
  if (!tags || tags.length === 0) return;
  const filteredTags = tags.filter((t) => t.trim());
  if (filteredTags.length === 0) return;

  const tagH = Math.round(canvasH * 0.115);
  const tagRadius = Math.round(tagH * 0.38);
  const tagGap = Math.round(canvasH * 0.038);
  const tagFontSize = Math.round(canvasH * 0.056);
  const tagPadX = Math.round(tagH * 0.5);

  const col2W = Math.round((maxWidth - tagGap) / 2);

  ctx.save();
  ctx.font = `600 ${tagFontSize}px -apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif`;
  ctx.textBaseline = "middle";

  // Calculate total height to center vertically
  const rows = [];
  let i = 0;
  while (i < filteredTags.length) {
    const tag1 = filteredTags[i];
    const tag2 = filteredTags[i + 1];
    const w1 = ctx.measureText(tag1).width + tagPadX * 2;
    const w2 = tag2 ? ctx.measureText(tag2).width + tagPadX * 2 : 0;
    // If tag1 fits in half-width AND tag2 exists and fits, put them side by side
    if (tag2 && w1 <= col2W + 4 && w2 <= col2W + 4) {
      rows.push([tag1, tag2]);
      i += 2;
    } else {
      rows.push([tag1]);
  ...
  const totalH = rows.length * tagH + (rows.length - 1) * tagGap;
  let y = Math.round((canvasH - totalH) / 2);

  rows.forEach((row) => {
    if (row.length === 2) {
      // Two tags side by side
      [row[0], row[1]].forEach((tag, ci) => {
        const x = startX + ci * (col2W + tagGap);
        drawTagPill(ctx, x, y, col2W, tagH, tagRadius, tag, tagFontSize);
      });
    } else {
      // One tag - full width
      drawTagPill(
...
function drawTagPill(ctx, x, y, w, h, r, label, fontSize) {
  ctx.fillStyle = "rgba(255,255,255,0.15)";
  roundRect(ctx, x, y, w, h, r);
  ctx.fill();

  ctx.fillStyle = "#ffffff";
  ctx.font = `600 ${fontSize}px -apple-system, BlinkMacSystemFont, 'SF Pro Display', 'Helvetica Neue', sans-serif`;
  ctx.textBaseline = "middle";
  ctx.textAlign = "center";
  const textPad = Math.round(h * 0.4);
  ctx.fillText(label, x + w / 2, y + h / 2, w - textPad * 2);
  ctx.textAlign = "left";
}

// Helper: roundRect path (wraps ctx.roundRect with fallback)
...
/**
 * Parse a config string (JSON or ES-module JS with export const APP/SLIDES/FEATURE_GRAPHIC).
 * Returns a normalised { app, slides: [], featureGraphic } object or throws on error.
 */
function parseAppConfig(text) {
  ...
  if (text.startsWith("{")) {
    raw = JSON.parse(text); // throws on invalid JSON
    // Normalise: JSON may already use { app, slides, featureGraphic }
    // or it may mirror the JS shape { APP, SLIDES, FEATURE_GRAPHIC }
    if (!raw.slides && !raw.SLIDES) {
      throw new Error(
        'JSON içinde "slides" (veya "SLIDES") dizisi bulunamadı.',
      );
    }
  ...
  // ── Try ES-module JS ─────────────────────────────────────
  // Strip export keyword so the declarations become plain vars
  const stripped = text.replace(/export\s+const\s+/g, "var ");
  let result;
  try {
    // Use Function constructor: safer than eval, still executes in strict context
    // We collect all three possible exports regardless of whether they exist
    const fn = new Function(
      stripped +
        '\n;return (typeof FEATURE_GRAPHIC !== "undefined" ? {APP: typeof APP !== "undefined" ? APP : null, SLIDES: typeof SLIDES !== "undefined" ? SLIDES : null, FEATURE_GRAPHIC: FEATURE_GRAPHIC} : {APP: typeof APP !== "undefined" ? APP : null, SLIDES: typeof SLIDES !== "undefined" ? SLIDES : null, FEATURE_GRAPHIC: null});',
    );
    result = fn();
  } catch (e) {
    throw new Error("JS parse hatası: " + e.message);
  }
  if (!result.APP && !result.SLIDES && !result.FEATURE_GRAPHIC) {
    throw new Error("APP, SLIDES veya FEATURE_GRAPHIC export'u bulunamadı.");
  }
  return normaliseRawConfig(result);
}

function normaliseRawConfig(raw) {
  // Support both camelCase (JSON) and UPPER_CASE (JS exports)
  const app = raw.app || raw.APP || {};
  const slidesRaw = raw.slides || raw.SLIDES;
  const fg = raw.featureGraphic || raw.FEATURE_GRAPHIC || null;

  let slides = [];
  if (Array.isArray(slidesRaw)) {
    slides = slidesRaw;
  } else if (slidesRaw && typeof slidesRaw === "object") {
    // Object keyed by slide name (like the user's SLIDES constant)
    slides = Object.values(slidesRaw);
  }

  return { app, slides, featureGraphic: fg };
}

/**
 * Apply a parsed config object to the current project and its screenshots.
 * - Updates project name / tagline / featureTags
 * - Applies slides[i] text + style to screenshots[i] (by order)
 * - Saves state
 */
function applyAppConfig(config) {
  const project = getCurrentProject();

  // 1. Project meta from featureGraphic (takes precedence) or app
  const fg = config.featureGraphic;
  const appCfg = config.app || {};

  const newName = (fg && fg.appName) || appCfg.name;
  if (newName) {
    project.name = newName;
    // keep projects array in sync
    const pEntry = projects.find((p) => p.id === project.id);
    if (pEntry) pEntry.name = newName;
  }

  const newTagline =
    fg && fg.tagline !== undefined
      ? fg.tagline
      : appCfg.tagline !== undefined
        ? appCfg.tagline
        : undefined;
  if (newTagline !== undefined) project.tagline = newTagline;

  if (fg && Array.isArray(fg.tags)) {
    project.featureTags = [...fg.tags];
  }

  // 2. Apply slides (by order) to existing screenshots
  const slides = config.slides || [];
  slides.forEach((slide, i) => {
    if (i >= state.screenshots.length) return;
    const ss = state.screenshots[i];

    // Normalise text object (ensures all fields exist)
    ss.text = normalizeTextSettings(ss.text);

    // ── Headline ──────────────────────────────────
    const headlineRaw = slide.headline;
    if (headlineRaw !== undefined) {
      const headlineStr = Array.isArray(headlineRaw)
        ? headlineRaw.join("\n")
        : String(headlineRaw);
      ss.text.headlineEnabled = true;
      const langs = ss.text.headlineLanguages || ["en"];
      langs.forEach((lang) => {
        if (!ss.text.headlines) ss.text.headlines = {};
        ss.text.headlines[lang] = headlineStr;
      });
    }

    // ── Subheadline (sub field) ───────────────────
    const subRaw = slide.sub || slide.subheadline || slide.badge || null;
    if (subRaw) {
      ss.text.subheadlineEnabled = true;
      const langs = ss.text.subheadlineLanguages || ["en"];
      langs.forEach((lang) => {
        if (!ss.text.subheadlines) ss.text.subheadlines = {};
        ss.text.subheadlines[lang] = subRaw;
      });
    }

    // ── Style ─────────────────────────────────────
    if (slide.titleColor) ss.text.headlineColor = slide.titleColor;
    if (slide.titleFont) ss.text.headlineFont = slide.titleFont;
    if (slide.titleFontWeight != null)
      ss.text.headlineWeight = String(slide.titleFontWeight);
    if (slide.titleFontSize != null)
      ss.text.headlineSize = Number(slide.titleFontSize);
    if (slide.titleLetterSpacing != null)
      ss.text.letterSpacing = Number(slide.titleLetterSpacing);
    if (slide.titleLineHeight != null && slide.titleFontSize) {
      // Convert absolute px lineHeight → percentage relative to fontSize
      ss.text.lineHeight = Math.round(
        (slide.titleLineHeight / slide.titleFontSize) * 100,
      );
    }
  });
  ...
  return {
    ...
    defaults: {
      ...
      background: {
        ...
        image: undefined,
        imageSrc:
          state.defaults.background.imageSrc ||
          state.defaults.background.image?.src ||
          null,
      },
    },
  };
...
function exportProject() {
  ...
  a.href = url;
  a.download =
    (project?.name || "project").replace(/[^a-zA-Z0-9_-]/g, "_") + ".appscreen";
...
async function handleProjectImport(file) {
  ...
  try {
    ...
    if (!data.screenshots || !Array.isArray(data.screenshots)) {
      await showAppAlert(
        "Invalid project file: missing screenshots data",
        "error",
      );
      return;
    }
    ...
    const projectName =
      data.projectName ||
      file.name.replace(/\.appscreen$|\.json$/, "") ||
      "Imported Project";
    ...
    transaction.oncomplete = async () => {
      ...
      updateProjectSelector();
      await showAppAlert(
        `Project "${projectName}" imported successfully`,
        "success",
      );
    };
...
function updateFrameColorSwatches(deviceType, activeColorId) {
  ...
  const presets =
    typeof frameColorPresets !== "undefined"
      ? frameColorPresets[deviceType]
      : null;
  if (!presets) {
    container.innerHTML = "";
    return;
  ...
  container.querySelectorAll(".frame-color-swatch").forEach((swatch) => {
    swatch.addEventListener("click", () => {
      ...
      container
        .querySelectorAll(".frame-color-swatch")
...
function syncUIWithState() {
  ...
  // Update feature graphic panel visibility
  updateFeatureGraphicPanel();
  ...
  document.getElementById("position-presets-section").style.display = use3D
    ? "none"
    : "block";
  document.getElementById("frame-color-section").style.display = use3D
    ? "block"
    : "none";
...
function setupEventListeners() {
  ...
  // Project logo upload
  document.getElementById("project-logo-btn").addEventListener("click", (e) => {
    e.stopPropagation(); // prevent project dropdown from opening
    document.getElementById("project-logo-input").click();
  });
  document
    .getElementById("project-logo-input")
    .addEventListener("change", (e) => {
      ...
      reader.onload = (ev) => {
        const project = getCurrentProject();
        if (project) {
          project.logo = ev.target.result;
          loadProjectLogoImage(project);
          saveProjectsMeta();
          updateProjectLogoUI();
          if (state.outputDevice === "web-feature") updateCanvas();
        }
  ...
  // Feature tagline input
  document.getElementById("feature-tagline").addEventListener("input", (e) => {
    const project = getCurrentProject();
    if (project) {
      project.tagline = e.target.value;
      saveProjectsMeta();
      if (state.outputDevice === "web-feature") updateCanvas();
    }
  });

  // Add feature tag button
  document
    .getElementById("add-feature-tag-btn")
    .addEventListener("click", () => {
      const project = getCurrentProject();
      if (!project) return;
      if (!project.featureTags) project.featureTags = [];
      project.featureTags.push("");
      saveProjectsMeta();
      renderFeatureTagsList();
    });
  ...
  if (addTemplateBtn) {
    addTemplateBtn.addEventListener("click", () => showTemplatesModal());
  }
  ...
  // ── App Config Import ─────────────────────────────────────────────────────
  (function setupAppConfigModal() {
    const modal = document.getElementById("app-config-modal");
    const openBtn = document.getElementById("import-config-btn");
    const closeBtn = document.getElementById("app-config-modal-close");
    const cancelBtn = document.getElementById("app-config-modal-cancel");
    const applyBtn = document.getElementById("app-config-apply-btn");
    const pasteInput = document.getElementById("config-paste-input");
    const fileInput = document.getElementById("config-file-input");
    const fileLabel = document.getElementById("config-file-label-text");
    const errorDiv = document.getElementById("config-error-msg");
    const previewDiv = document.getElementById("config-preview-msg");
    const tabs = document.querySelectorAll(".config-tab");
    const panes = {
      paste: document.getElementById("config-pane-paste"),
      file: document.getElementById("config-pane-file"),
    };
    ...
    function showPreview(cfg) {
      const slideCount = (cfg.slides || []).length;
      const name =
        (cfg.featureGraphic && cfg.featureGraphic.appName) ||
        (cfg.app && cfg.app.name) ||
        "—";
      const tagCount =
        cfg.featureGraphic && cfg.featureGraphic.tags
          ? cfg.featureGraphic.tags.length
          : 0;
      previewDiv.innerHTML =
        `<strong>Hazır:</strong> Uygulama adı: <em>${name}</em> · ` +
        `${slideCount} slide · ${tagCount} Feature Graphic tag`;
    ...
    tabs.forEach((tab) => {
      tab.addEventListener("click", () => {
        ...
        panes.paste.style.display = activeTab === "paste" ? "flex" : "none";
        panes.file.style.display = activeTab === "file" ? "flex" : "none";
        errorDiv.style.display = "none";
        previewDiv.style.display = "none";
      });
    });
    ...
    fileInput.addEventListener("change", () => {
      ...
      reader.onload = (e) => {
        ...
        // Try to parse immediately and show preview
        try {
          const cfg = parseAppConfig(pendingFileText);
          showPreview(cfg);
        } catch (err) {
    ...
    applyBtn.addEventListener("click", () => {
      ...
      if (activeTab === "paste") {
        ...
        if (!text) {
          showError("Lütfen config içeriğini yapıştırın.");
          return;
        }
      } else {
        if (!pendingFileText) {
...
function createNewScreenshot(img, src, name, lang, deviceType) {
  ...
  state.screenshots.push({
    ...
    background: (() => {
      const bg = JSON.parse(
        JSON.stringify({ ...state.defaults.background, image: undefined }),
      );
      if (bg.imageSrc) {
        const bgImg = new Image();
        bgImg.onload = () => {
...
function updateCanvas() {
  ...
  // Feature Graphic mode: draw special layout instead of normal screenshot+text pipeline
  if (state.outputDevice === "web-feature") {
    drawFeatureGraphicLayout();
    updateSidePreviews();
    return;
  }
...
const DESIGN_TEMPLATES = [
  // ─── 1. DARK STAGE — Spotify/Apple Music style ───────────────────────────
  // Phone fills top 60%, HUGE bold text slams the bottom
  {
    id: "dark-stage",
    name: "Dark Stage",
    description: "Bold bottom text · Spotify style",
    previewColors: ["#0a0a0a", "#12001a", "#0a0a0a"],
    previewAngle: 180,
    background: {
      type: "gradient",
      gradient: {
    ...
    text: {
      ...
      headlineSize: 100,
      headlineWeight: "900",
      headlineColor: "#ff2d9b",
      position: "bottom",
      offsetY: 9,
      lineHeight: 104,
      subheadlineEnabled: false,
      subheadlineFont: "'Poppins', sans-serif",
      subheadlineSize: 44,
      subheadlineWeight: "400",
      subheadlineColor: "#ff9de2",
      subheadlineOpacity: 80,
    },
    slides: [
      { headline: "FEEL THE\nMUSIC VIBE", subheadline: "" },
      { headline: "DISCOVER\nNEW SOUNDS", subheadline: "" },
      { headline: "TOP HITS\nALL DAY", subheadline: "" },
      { headline: "LISTEN\nANYWHERE", subheadline: "" },
      { headline: "YOUR\nSTAGE", subheadline: "" },
    ],
  },

  // ─── 2. MINIMAL LIGHT — WhatsApp/Telegram style ──────────────────────────
  // Bright white background, dark text at top, clean phone at bottom
  {
    id: "minimal-light",
    name: "Minimal Light",
    description: "Clean white · WhatsApp style",
    previewColors: ["#ffffff", "#f0f4ff", "#e8ecf8"],
    previewAngle: 160,
    background: {
      type: "gradient",
      gradient: {
    ...
    slides: [
      {
        headline: "Stay Close\nAnywhere",
        subheadline: "Message friends instantly",
      },
      {
        headline: "Fast &\nSecure",
        subheadline: "End-to-end encrypted always",
      },
      {
        headline: "Share\nEverything",
        subheadline: "Photos, videos, documents",
      },
      { headline: "Group\nChats", subheadline: "Bring your people together" },
      { headline: "Free\nForever", subheadline: "No subscriptions needed" },
    ],
  },

  // ─── 3. VIVID GRADIENT — Instagram/Canva/TikTok style ───────────────────
  // Explosive gradient background, bold white text at top, phone at bottom
  {
    id: "vivid-gradient",
    name: "Vivid Gradient",
    description: "Explosive color · Instagram style",
    previewColors: ["#7b2ff7", "#f107a3", "#ff6b35"],
    previewAngle: 135,
    background: {
      type: "gradient",
      gradient: {
    ...
    text: {
      ...
      subheadlineColor: "#ffe0f0",
      subheadlineOpacity: 90,
    },
    slides: [
      {
        headline: "Create\nBeautifully",
        subheadline: "Design that stands out",
      },
      {
        headline: "Share\nYour Story",
        subheadline: "Reach millions instantly",
      },
      { headline: "Grow Your\nAudience", subheadline: "Analytics that matter" },
      {
        headline: "Collab\nAnywhere",
        subheadline: "Teams work better together",
      },
      { headline: "Go Live\nNow", subheadline: "Stream to your followers" },
    ],
  },

  // ─── 4. TILT & GLOW — Gaming/Music app style ─────────────────────────────
  // Phone rotated -8°, skewed with perspective, neon teal accent on black
  {
    id: "tilt-glow",
    name: "Tilt & Glow",
    description: "Angled phone · Gaming style",
    previewColors: ["#050b0e", "#071820", "#040c12"],
    previewAngle: 150,
    background: {
      type: "gradient",
      gradient: {
        angle: 150,
        stops: [
          { color: "#050b0e", position: 0 },
          { color: "#071820", position: 55 },
          { color: "#040c12", position: 100 },
        ],
      },
      solid: "#050b0e",
      image: null,
    ...
    text: {
      ...
      headlineSize: 88,
      headlineWeight: "800",
      headlineColor: "#00f5d4",
      position: "top",
      offsetY: 9,
      lineHeight: 107,
      subheadlineEnabled: true,
      subheadlineFont: "'Manrope', sans-serif",
      subheadlineSize: 44,
      subheadlineWeight: "400",
      subheadlineColor: "#80fff0",
      subheadlineOpacity: 72,
    },
    slides: [
      {
        headline: "NEXT LEVEL\nGAMEPLAY",
        subheadline: "Compete with players worldwide",
      },
      {
        headline: "BATTLE\nREAL-TIME",
        subheadline: "Zero lag, pure adrenaline",
      },
      { headline: "WIN\nBIG DAILY", subheadline: "Tournaments every hour" },
      { headline: "EPIC\nGRAPHICS", subheadline: "Console quality on mobile" },
      { headline: "PLAY\nFREE NOW", subheadline: "No hidden costs, ever" },
    ],
  },

  // ─── 5. BOTTOM BOLD — YouTube/Netflix style ──────────────────────────────
  // Phone with perspective skew fills the top, HUGE white text at bottom + red sub
  {
    id: "bottom-bold",
    name: "Bottom Bold",
    description: "Giant bottom text · YouTube style",
    previewColors: ["#0f0f0f", "#1a0505", "#0f0f0f"],
    previewAngle: 180,
    background: {
      type: "gradient",
      gradient: {
    ...
    text: {
      ...
      lineHeight: 104,
      subheadlineEnabled: true,
      subheadlineFont: "'Plus Jakarta Sans', sans-serif",
      subheadlineSize: 46,
      subheadlineWeight: "400",
      subheadlineColor: "#ff4444",
      subheadlineOpacity: 95,
    },
    slides: [
      {
        headline: "Watch\nAnywhere",
        subheadline: "Stream on all your devices",
      },
      {
        headline: "Never\nMiss Out",
        subheadline: "Subscribe to your favorites",
      },
      { headline: "Go\nPremium", subheadline: "Ad-free · Offline · Unlimited" },
      {
        headline: "Start\nCreating",
        subheadline: "Upload your first video today",
      },
      {
        headline: "Billions\nOf Videos",
        subheadline: "Find anything, instantly",
      },
    ],
  },

  // ─── 6. PREMIUM ANGLE — Apple/Google Maps style ──────────────────────────
  // Deep blue, phone slightly tilted right (+6°) with glow, text at bottom
  {
    id: "premium-angle",
    name: "Premium Angle",
    description: "Tilted phone · Apple Maps style",
    previewColors: ["#000428", "#003d82", "#000f2e"],
    previewAngle: 140,
    background: {
      type: "gradient",
      gradient: {
        ...
        stops: [
          ...
          { color: "#003d82", position: 60 },
          { color: "#000f2e", position: 100 },
        ],
      },
      solid: "#000428",
      image: null,
    ...
    text: {
      ...
      lineHeight: 110,
      subheadlineEnabled: true,
      subheadlineFont: "'DM Sans', sans-serif",
      subheadlineSize: 44,
      subheadlineWeight: "300",
      subheadlineColor: "#7eb8ff",
      subheadlineOpacity: 90,
    },
    slides: [
      {
        headline: "Navigate\nSmarter",
        subheadline: "Real-time traffic worldwide",
      },
      {
        headline: "Find It\nInstantly",
        subheadline: "Search 200M+ places globally",
      },
      {
        headline: "Explore\nNearby",
        subheadline: "Discover hidden gems around you",
      },
      {
        headline: "Share\nYour Trip",
        subheadline: "Live location with loved ones",
      },
      {
        headline: "Drive\nSmarter",
        subheadline: "Traffic avoided automatically",
      },
    ],
  },

  // ─── 7. APPLE LIGHT — iOS / Apple-style clean ────────────────────────────
  {
    id: "apple-light",
    name: "Apple Light",
    description: "Ultra clean · iOS style",
    previewColors: ["#ffffff", "#f5f5f7", "#e8e8ed"],
    previewAngle: 180,
    isLight: true,
    background: {
      type: "gradient",
    ...
    text: {
      ...
      subheadlineSize: 42,
      subheadlineWeight: "400",
      subheadlineColor: "#6e6e73",
      subheadlineOpacity: 100,
    },
    slides: [
      {
        headline: "Simple\nBeautiful",
        subheadline: "Design that gets out of the way",
      },
      {
        headline: "Works on\nEvery Device",
        subheadline: "Seamless across iPhone & iPad",
      },
      {
        headline: "Private\nBy Design",
      ...
      {
        headline: "Instant\nSetup",
        subheadline: "Ready in seconds, not minutes",
      },
      {
        headline: "Free to\nGet Started",
  ...
  // ─── 8. CORAL ENERGY — Fitness / Health warm ─────────────────────────────
  {
    id: "coral-energy",
    name: "Coral Energy",
    description: "Warm vibes · Fitness style",
    previewColors: ["#ff6b35", "#f7b731", "#ff4757"],
    previewAngle: 135,
    background: {
      type: "gradient",
      gradient: {
    ...
    text: {
      ...
      subheadlineSize: 44,
      subheadlineWeight: "400",
      subheadlineColor: "#fff5e0",
      subheadlineOpacity: 90,
    },
    slides: [
      {
        headline: "Train\nHarder",
        subheadline: "Workouts built for your goals",
      },
      { headline: "Burn\nMore", subheadline: "Real-time calorie tracking" },
      { headline: "Track\nProgress", subheadline: "See how far you've come" },
      {
        headline: "Stay\nMotivated",
        subheadline: "Daily streaks & challenges",
      },
      { headline: "Run\nFurther", subheadline: "GPS routes for every level" },
    ],
  },

  // ─── 9. MIDNIGHT GOLD — Finance / Premium luxury ─────────────────────────
  {
    id: "midnight-gold",
    name: "Midnight Gold",
    description: "Luxury dark · Finance style",
    previewColors: ["#0a0800", "#1a1400", "#0a0800"],
    previewAngle: 180,
    background: {
      type: "gradient",
      gradient: {
        angle: 180,
        stops: [
          { color: "#0a0800", position: 0 },
          { color: "#1a1400", position: 55 },
          { color: "#0a0800", position: 100 },
        ],
      },
      solid: "#0a0800",
      image: null,
    ...
    screenshot: {
      ...
      frame: { enabled: true, color: "#c9a227", width: 2, opacity: 30 },
    },
    text: {
      ...
      lineHeight: 110,
      subheadlineEnabled: true,
      subheadlineFont: "'Playfair Display', serif",
      subheadlineSize: 42,
      subheadlineWeight: "400",
      subheadlineColor: "#f0d080",
      subheadlineOpacity: 75,
    },
    slides: [
      {
        headline: "Grow Your\nWealth",
        subheadline: "Smart investments, made simple",
      },
      {
        headline: "Track\nEvery Cent",
        subheadline: "Full financial picture at a glance",
      },
      { headline: "Earn\nMore", subheadline: "Rewards on every transaction" },
      {
        headline: "Invest With\nConfidence",
        subheadline: "Backed by market-leading AI",
      },
      {
        headline: "Your Money\nWorks Harder",
        subheadline: "Automated savings & returns",
      },
    ],
  },

  // ─── 10. NEON POP — Social / Entertainment bold ──────────────────────────
  {
    id: "neon-pop",
    name: "Neon Pop",
    description: "Bold yellow · Social style",
    previewColors: ["#120024", "#2d1b69", "#0d001a"],
    previewAngle: 145,
    background: {
      type: "gradient",
      gradient: {
        angle: 145,
        stops: [
          { color: "#120024", position: 0 },
          { color: "#2d1b69", position: 60 },
          { color: "#0d001a", position: 100 },
        ],
      },
      solid: "#120024",
      image: null,
    ...
    text: {
      ...
      subheadlineWeight: "400",
      subheadlineColor: "#ffffff",
      subheadlineOpacity: 65,
    },
    slides: [
      {
        headline: "Go Live\nRight Now",
        subheadline: "Stream to millions instantly",
      },
      {
        headline: "Trending\nContent",
        subheadline: "Discover what's viral today",
      },
      {
        headline: "Follow Your\nCreators",
        subheadline: "Never miss a new post again",
      },
      {
        headline: "React &\nShare",
        subheadline: "Express yourself in real-time",
      },
      {
        headline: "Grow Your\nFan Base",
        subheadline: "Built-in tools for creators",
      },
    ],
  },

  // ─── 11. OCEAN WAVE — Travel / Lifestyle ─────────────────────────────────
  {
    id: "ocean-wave",
    name: "Ocean Wave",
    description: "Deep blue · Travel style",
    previewColors: ["#006994", "#004e7c", "#001f3d"],
    previewAngle: 155,
    background: {
      type: "gradient",
      gradient: {
        ...
        stops: [
          ...
          { color: "#004e7c", position: 45 },
          { color: "#001f3d", position: 100 },
        ],
      },
      solid: "#006994",
      image: null,
    ...
    text: {
      ...
      subheadlineSize: 43,
      subheadlineWeight: "400",
      subheadlineColor: "#7ec8e3",
      subheadlineOpacity: 90,
    },
    slides: [
      {
        headline: "Explore\nThe World",
        subheadline: "500K+ destinations mapped",
      },
      {
        headline: "Book in\nSeconds",
        subheadline: "Best rates guaranteed always",
      },
      { headline: "Travel\nSmarter", subheadline: "AI trip planner built-in" },
      { headline: "Offline\nMaps", subheadline: "Navigate without internet" },
      {
        headline: "Share\nJourneys",
        subheadline: "Connect with fellow travelers",
      },
    ],
  },

  // ─── 12. SAGE CALM — Wellness / Meditation ───────────────────────────────
  {
    id: "sage-calm",
    name: "Sage Calm",
    description: "Natural green · Wellness style",
    previewColors: ["#e8f5e8", "#d4e6c3", "#f0f4ec"],
    previewAngle: 160,
    isLight: true,
    background: {
      type: "gradient",
      gradient: {
        angle: 160,
        stops: [
          { color: "#e8f5e8", position: 0 },
          { color: "#d4e6c3", position: 50 },
          { color: "#f0f4ec", position: 100 },
        ],
      },
      solid: "#e8f5e8",
      image: null,
    ...
    text: {
      ...
      subheadlineSize: 42,
      subheadlineWeight: "400",
      subheadlineColor: "#3d6b3d",
      subheadlineOpacity: 90,
    },
    slides: [
      {
        headline: "Find Your\nInner Peace",
        subheadline: "Guided meditation for every mood",
      },
      {
        headline: "Breathe\nDeep",
        subheadline: "Stress relief in just 5 minutes",
      },
      {
        headline: "Sleep\nBetter",
        subheadline: "Bedtime stories & sleep sounds",
      },
      {
        headline: "Daily\nMindfulness",
        subheadline: "Build a habit that sticks",
      },
      {
        headline: "Feel\nCalm Today",
        subheadline: "Thousands love it worldwide",
      },
    ],
  },
];
...
function renderTemplateGrid() {
  ...
  grid.innerHTML = DESIGN_TEMPLATES.map((tpl) => {
    const gradientStr = tpl.previewColors.join(", ");
    const gradient = `linear-gradient(${tpl.previewAngle}deg, ${gradientStr})`;
    const isLight = tpl.isLight === true;
    const textPos = tpl.text?.position || "top";
    const lineColor1 = isLight
      ? "rgba(30,41,59,0.55)"
      : "rgba(255,255,255,0.88)";
    const lineColor2 = isLight
      ? "rgba(30,41,59,0.3)"
      : "rgba(255,255,255,0.45)";
    const phoneOpacity = isLight ? "0.55" : "0.38";

    const linesHTML = `
      <div class="tpl-line tpl-line-1" style="background:${lineColor1}"></div>
      <div class="tpl-line tpl-line-2" style="background:${lineColor2}"></div>
    `;
    const phoneHTML = `<div class="template-preview-phone" style="opacity:${phoneOpacity}"></div>`;

    const contentHTML =
      textPos === "bottom"
        ? `${phoneHTML}<div class="template-preview-lines tpl-lines-bottom">${linesHTML}</div>`
        : `<div class="template-preview-lines">${linesHTML}</div>${phoneHTML}`;
...
async function applySelectedTemplate() {
  const grid = document.getElementById("template-grid");
  if (!grid) return;
  const selected = grid.querySelector(".template-card.selected");
  if (!selected) {
    await showAppAlert("Please select a template first.", "info");
    return;
  }
  ...
  hideTemplatesModal();
  await applyDesignTemplate(tpl);
}

/**
 * Normalizes template screenshot + text settings for the current output device.
 * Templates are tuned for iPhone 6.9" (1320×2868). This adapts font sizes
 * proportionally to canvas height so text never overlaps the phone screenshot.
 * For landscape/web canvases it also disables tilt and recentres the layout.
 */
function normalizeTemplateForDevice(tpl) {
  const deviceKey = state.outputDevice;
  const dims =
    deviceKey === "custom"
      ? { width: state.customWidth, height: state.customHeight }
      : deviceDimensions[deviceKey] || deviceDimensions["iphone-6.9"];

  const REF_H = 2868; // iPhone 6.9" canvas height – the design reference
  const devAR = dims.width / dims.height;
  const heightRatio = dims.height / REF_H;
  const isLandscape = devAR > 0.95;

  const ss = JSON.parse(JSON.stringify(tpl.screenshot));
  const txt = JSON.parse(JSON.stringify(tpl.text));

  // --- Font size: scale proportionally to canvas height ---
  // Clamp: never larger than reference (1.0) and never smaller than 35% (readability)
  const fontScale = Math.max(0.35, Math.min(1.0, heightRatio));
  txt.headlineSize = Math.round(tpl.text.headlineSize * fontScale);
  if (tpl.text.subheadlineSize) {
    txt.subheadlineSize = Math.round(tpl.text.subheadlineSize * fontScale);
  }

  if (isLandscape) {
    // Landscape / web canvases: centre the phone, no tilt effects
    ss.y = 50;
    ss.rotation = 0;
    ss.perspective = 0;
  }

  return { screenshot: ss, text: txt };
}

async function applyDesignTemplate(tpl) {
  // Load fonts and wait for them before rendering
  const fontsToLoad = [tpl.text.headlineFont, tpl.text.subheadlineFont].filter(
    Boolean,
  );
  await Promise.all(
    fontsToLoad.map((fontValue) => {
      const match = fontValue.match(/'([^']+)'/);
      return match ? loadGoogleFont(match[1]) : Promise.resolve();
    }),
  );

  // Adapt template values to the current output device
  const normalized = normalizeTemplateForDevice(tpl);
  const ntxt = normalized.text; // font-size-adjusted text settings

  // Build new defaults
  const newBg = JSON.parse(JSON.stringify(tpl.background));
  const newSs = normalized.screenshot;

  // Merge text settings over existing defaults (preserve headlines/subheadlines content)
  const currentText = JSON.parse(JSON.stringify(state.defaults.text));
  const newText = {
    ...currentText,
    headlineEnabled: true,
    headlineFont: tpl.text.headlineFont,
    headlineSize: ntxt.headlineSize,
    headlineWeight: tpl.text.headlineWeight,
    headlineColor: tpl.text.headlineColor,
    position: tpl.text.position,
    offsetY: tpl.text.offsetY,
    lineHeight: tpl.text.lineHeight,
    subheadlineEnabled: tpl.text.subheadlineEnabled,
    subheadlineFont: tpl.text.subheadlineFont || tpl.text.headlineFont,
    subheadlineSize: ntxt.subheadlineSize,
    subheadlineWeight: tpl.text.subheadlineWeight,
    subheadlineColor: tpl.text.subheadlineColor,
    subheadlineOpacity: tpl.text.subheadlineOpacity,
  };

  // Update languageSettings for each language to reflect new sizes/position
  const langs = newText.headlineLanguages || ["en"];
  langs.forEach((lang) => {
    if (!newText.languageSettings) newText.languageSettings = {};
    newText.languageSettings[lang] = {
      headlineSize: ntxt.headlineSize,
      subheadlineSize: ntxt.subheadlineSize,
      position: tpl.text.position,
      offsetY: tpl.text.offsetY,
  ...
  // Apply to defaults
  state.defaults.background = newBg;
  state.defaults.screenshot = newSs;
  state.defaults.text = newText;

  const hasScreenshots = state.screenshots.length > 0;
  const hasSlides = tpl.slides && tpl.slides.length > 0;

  if (hasScreenshots) {
    // Apply STYLE to all existing screenshots; preserve existing text content
    state.screenshots.forEach((ss, i) => {
      ss.background = JSON.parse(JSON.stringify(newBg));
      ss.screenshot = JSON.parse(JSON.stringify(newSs));

      const ssText = JSON.parse(JSON.stringify(ss.text));

      // Check if screenshot already has meaningful text in any language
      const hasExistingHeadline =
        ssText.headlines &&
        Object.values(ssText.headlines).some((v) => v && v.trim().length > 0);
      const hasExistingSubheadline =
        ssText.subheadlines &&
        Object.values(ssText.subheadlines).some(
          (v) => v && v.trim().length > 0,
        );

      ss.text = {
        ...ssText,
        // Style properties — always updated
        headlineFont: tpl.text.headlineFont,
        headlineSize: ntxt.headlineSize,
        headlineWeight: tpl.text.headlineWeight,
        headlineColor: tpl.text.headlineColor,
        ...
        subheadlineColor: tpl.text.subheadlineColor,
        subheadlineOpacity: tpl.text.subheadlineOpacity,
        // Enable subheadline if it was disabled and we have slide text to fill
        subheadlineEnabled:
          hasExistingSubheadline ||
          (!hasExistingHeadline && tpl.text.subheadlineEnabled)
            ? ssText.subheadlineEnabled
            : tpl.text.subheadlineEnabled,
        // Preserve enable state for headline if there's existing content
        headlineEnabled:
          ssText.headlineEnabled !== false
            ? true
            : hasExistingHeadline
              ? true
              : ssText.headlineEnabled,
      };

      // Update per-language layout settings
      const ssLangs = ss.text.headlineLanguages || ["en"];
      ssLangs.forEach((lang) => {
        if (!ss.text.languageSettings) ss.text.languageSettings = {};
        ss.text.languageSettings[lang] = {
          headlineSize: ntxt.headlineSize,
          subheadlineSize: ntxt.subheadlineSize,
          position: tpl.text.position,
          offsetY: tpl.text.offsetY,
      ...
      // Only fill in template text if the screenshot has NO existing text
      if (!hasExistingHeadline && hasSlides && i < tpl.slides.length) {
        const slide = tpl.slides[i];
        const headlineLangs = ss.text.headlineLanguages || ["en"];
        headlineLangs.forEach((lang) => {
          if (!ss.text.headlines) ss.text.headlines = {};
          ss.text.headlines[lang] = slide.headline;
        });
        ss.text.headlineEnabled = true;
      }
      if (!hasExistingSubheadline && hasSlides && i < tpl.slides.length) {
        const slide = tpl.slides[i];
        if (slide.subheadline) {
          const subLangs = ss.text.subheadlineLanguages || ["en"];
          subLangs.forEach((lang) => {
            if (!ss.text.subheadlines) ss.text.subheadlines = {};
            ss.text.subheadlines[lang] = slide.subheadline;
          });
          ss.text.subheadlineEnabled = true;
        }
      }
    ...
    // No screenshots yet — create blank screens with slide text
    tpl.slides.forEach((slide) => {
      createNewScreenshot(null, null, "Blank Screen", null, state.outputDevice);
      const ss = state.screenshots[state.screenshots.length - 1];
      ss.background = JSON.parse(JSON.stringify(newBg));
      ss.screenshot = JSON.parse(JSON.stringify(newSs));
      ss.text = JSON.parse(JSON.stringify(newText));
      const headlineLangs = ss.text.headlineLanguages || ["en"];
      headlineLangs.forEach((lang) => {
        ss.text.headlines[lang] = slide.headline;
        if (slide.subheadline && ss.text.subheadlines) {
          ss.text.subheadlines[lang] = slide.subheadline;
        }
      });
    });
  ...
  updateScreenshotList();
  updateCanvas();
  // Re-render once more after a short delay to ensure fonts are painted
  setTimeout(() => updateCanvas(), 150);
}
